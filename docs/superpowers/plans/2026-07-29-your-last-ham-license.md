...
**Tech Stack:** Python 3 (stdlib + `edge-tts`, `matplotlib`), headless `google-chrome` for PDF, `ffmpeg` for audio, nginx/Docker, GitHub Actions → GHCR. Base for all copying: `/home/kasm-user/your-next-ham-license/` ("Book 3", the General repo — the newest toolchain; **NOT** `your-first-ham-license`). Design spec: `docs/superpowers/specs/2026-07-29-your-last-ham-license-design.md` (approved).
