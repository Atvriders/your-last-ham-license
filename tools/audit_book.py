"""Verification harness ("test suite") for the finished Your Last Ham License book.

Cross-checks the book against the accuracy canon, the 2024–2028 Extra
...
depend on missing inputs are skipped, not failed):
    ...
    canon/pool-extra.json      -- structured pool: id -> {group, subelement,
...
POOL_JSON_PATH = "canon/pool-extra.json"
...
# wider than the General pool's A--F; the ingestion's group list (plan Task
# 2.1) is authoritative for this letter class.
_POOL_QUOTE_RE = re.compile(r"^\s*>\s*\*\*(E\d[A-H]\d\d)\*\*\s+(.+?)\s*$")
...
def extract_pool_quotes(text: str) -> list[dict]:
    ...
    Each block starts with a line ``> **E#X##** <question>`` and continues
...
def main() -> int:
    ...
    print("=== Your Last Ham License: book audit ===\n")
