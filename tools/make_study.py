"""Interactive study pages for Your Last Ham License.
...
external references:
  ...
  practice.html    -- draws a valid Extra exam: exactly one question per
                      NCVEC group (50 groups -> 50 questions, 37 to pass),
                      click-to-answer, grading with score, pass/fail and a
                      per-question review (your answer, the correct answer,
                      and the one-line why), plus a per-subelement drill
                      mode with immediate feedback.
  flashcards.html  -- all 599 questions as flippable cards: front = question
                      ...
                      mark persisted in localStorage ("ylhl-study").
...
``canon/pool-extra.txt``, and ``chapter`` follows the subelement -> chapter
map of ``accuracy-canon.md`` §5 (one subelement per chapter: E1 -> ch01 …
E9 -> ch09, E0 -> ch10; no splits). The 28 figure questions get their
redrawn pool figure SVG (``figures/chNN-pool-fig-eNN.svg``) embedded inline.

Usage:
  python3 tools/make_study.py [--out build/]
      [--pool canon/pool-extra.json] [--appendix appendices/pool.md]
      [--pool-txt canon/pool-extra.txt] [--figures-dir figures]
"""
...
DEFAULT_POOL_PATH = "canon/pool-extra.json"
DEFAULT_APPENDIX_PATH = "appendices/pool.md"
DEFAULT_POOL_TXT_PATH = "canon/pool-extra.txt"
DEFAULT_FIGURES_DIR = "figures"

POOL_QUESTION_COUNT = 599

# The only pool questions that reference a figure (accuracy-canon.md §1.4):
# 28 questions across the ten pool figures.
FIGURE_QUESTION_IDS = frozenset({
    "E5C10", "E5C11", "E5C12",
    "E6A10", "E6A11", "E6B10", "E6C08", "E6C10", "E6C11",
    "E7B10", "E7B11", "E7B12", "E7D06", "E7D07", "E7D08",
    "E7G02", "E7G07", "E7G09", "E7G10", "E7G11",
    "E9B01", "E9B02", "E9B03", "E9B04", "E9B05", "E9B06",
    "E9G06", "E9G07",
})
...
FIGURE_FILES = {
    "E5-1": "ch05-pool-fig-e51.svg",
    "E6-1": "ch06-pool-fig-e61.svg",
    "E6-2": "ch06-pool-fig-e62.svg",
    "E6-3": "ch06-pool-fig-e63.svg",
    "E7-1": "ch07-pool-fig-e71.svg",
    "E7-2": "ch07-pool-fig-e72.svg",
    "E7-3": "ch07-pool-fig-e73.svg",
    "E9-1": "ch09-pool-fig-e91.svg",
    "E9-2": "ch09-pool-fig-e92.svg",
    "E9-3": "ch09-pool-fig-e93.svg",
}

# Subelement -> teaching chapter (accuracy-canon.md §5): one subelement per
# chapter, E1 -> ch01 … E9 -> ch09, E0 -> ch10. No splits (unlike the
# Technician book's T8).
_SUBELEMENT_CHAPTER = {
    "E1": 1, "E2": 2, "E3": 3, "E4": 4, "E5": 5,
    "E6": 6, "E7": 7, "E8": 8, "E9": 9, "E0": 10,
}

_GROUP_HEADING_RE = re.compile(r"^(E\d[A-H]) (\S.*\S)\s*$")
# E0's published banner quirk (canon §1): "SUBELEMENT E0 - SAFETY - [1 exam
# question - 1 group]" — trailing hyphen and lowercase bracket where the
# other nine read "[6 Exam Questions - 6 Groups]". The optional `` -`` keeps
# E0's title "SAFETY".
_SUBELEMENT_RE = re.compile(
    r"^SUBELEMENT (E\d) - (.+?)(?: -)? \[\d+ [Ee]xam [Qq]uestions? - \d+ [Gg]roups?\]\s*$")
_ENTRY_ID_RE = re.compile(r"^> \*\*(E\d[A-H]\d\d)\*\*")
_ANSWER_RE = re.compile(r"^> \*\*Answer: ([A-D])\*\* — (.*\S)\s*$")


def parse_group_headings(pool_txt: str) -> dict:
    """Published group headings from the canonical pool text: ``E1A`` ->
    "Frequency privileges; signal frequency range; ...". Question ID lines
    (``E1A01 (D)``) never match: a digit, not a space, follows the group
    letter there."""
    headings = {}
...
def parse_subelement_titles(pool_txt: str) -> dict:
    """Published subelement titles: ``E1`` -> ``COMMISSION RULES`` (case as
    published in the pool document). E0's published banner carries a
    trailing hyphen — ``SUBELEMENT E0 - SAFETY - [1 exam question -
    1 group]`` — which the pattern drops, yielding ``SAFETY``."""
    titles = {}
    for line in pool_txt.splitlines():
...
def chapter_for(subelement: str, group: str) -> int:
    """Chapter that teaches this question (accuracy-canon.md §5). The Extra
    map has no group-level splits — ``group`` is accepted for parity with
    the Technician generator's interface and ignored."""
...
def load_figures(figures_dir) -> dict:
    """The ten redrawn pool figures, inlined: ``E5-1`` -> SVG markup."""
    figures = {}
...
def subelement_summaries(records: list, titles: dict) -> list:
    """Per-subelement counts + published titles, ordered E1–E9 then E0."""
    counts: dict[str, int] = {}
    for rec in records:
        counts[rec["subelement"]] = counts.get(rec["subelement"], 0) + 1
    order = sorted(counts, key=lambda s: 10 if s == "E0" else int(s[1:]))
...
_SERIES_BAR = """<nav class="series-bar" aria-label="Books in this series">
  ...
  <a class="current" href="/extra/" aria-current="page">Extra</a>
</nav>
"""
...
<p class="rule">The real Extra exam draws <b>50 questions</b> from this pool — one
from each of the <b>50 groups</b> — and you need <b>37 to pass</b>. Draw a valid exam
