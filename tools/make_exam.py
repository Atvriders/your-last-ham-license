"""Practice-exam generator for Your Last Ham License.

Draws a valid Extra class practice exam from the structured question
pool (``canon/pool-extra.json``): exactly one question per NCVEC group
(~50 groups -> 50 questions on the real pool), uniform random within each
...
The Element 4 exam structure is parameterized, not hardcoded: the exam size
is whatever the pool's group count is (one draw per group, driven by the
JSON — deleted-ID gaps simply shrink a group's draw list), and the pass
threshold is the ``PASS_SCORE`` constant (37 of 50 on the real pool —
Technician/General were 26 of 35).
...
answer, figure}}`` — ids like ``E1A01`` … ``E0B##``; group letters reach H
(E7/E9 historically run to group H).
"""
...
# Element 4: 50 questions (one per pool group), 37 correct to pass.
PASS_SCORE = 37

_POOL_ID_RE = re.compile(r"^E(\d)([A-H])(\d\d)$")
...
def pool_sort_key(qid: str) -> tuple:
    """Canonical pool order: subelements E1–E9 then E0, group A–H, number."""
...
def draw_exam(pool: dict, seed=None) -> list[Question]:
    ...
    pool order. The exam size is the pool's group count (50 on the real
    Extra pool); groups with deleted-ID gaps (e.g. E4D after a withdrawal)
    simply draw from whichever ids remain.
    """
...
def render_key(questions: list[Question]) -> str:
    ...
    lines.append("")
    lines.append(f"{PASS_SCORE} correct to pass.")
    lines.append("")
    lines.append("## Subelement tally")
    tally: dict[str, int] = {}
    for q in questions:
        tally[q.subelement] = tally.get(q.subelement, 0) + 1
    for sub in sorted(tally, key=lambda s: (10 if s == "E0" else int(s[1:]) if s[1:].isdigit() else 99, s)):
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="Draw an Extra class practice exam from the question pool.")
    ...
    print(f"drew {len(questions)} questions ({groups} groups, {PASS_SCORE} to pass)"
