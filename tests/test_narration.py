...
# --- The Extra formula set: complex impedance (j-operator, rectangular and
# polar forms), phase, time constants, reflection coefficient, and the
# heavier Greek letters must all speak sanely in the audiobook.

def test_speak_math_complex_rectangular():
    assert speak_math("$Z = R + jX$") == "Z equals R plus j X"
    assert speak_math("$Z = 50 + j50$") == "Z equals 50 plus j 50"

def test_speak_math_complex_polar():
    assert speak_math("$Z = 70.7 \\angle 45^\\circ$") == "Z equals 70.7 angle 45 degrees"
    assert speak_math("$50 + j50 = 70.7 \\angle 45^\\circ$") == \
        "50 plus j 50 equals 70.7 angle 45 degrees"

def test_speak_math_impedance_magnitude_bars():
    assert speak_math("$|Z| = \\sqrt{R^2 + X^2}$") == \
        "magnitude of Z equals the square root of R squared plus X squared"

def test_speak_math_phase_angle():
    assert speak_math("$\\phi = \\arctan(X/R)$") == "phi equals arctan X over R"

def test_speak_math_time_constants():
    assert speak_math("$\\tau = RC$") == "tau equals R C"
    assert speak_math("$\\tau = L/R$") == "tau equals L over R"

def test_speak_math_reflection_coefficient():
    assert speak_math("$\\Gamma = \\frac{Z_L - Z_0}{Z_L + Z_0}$") == \
        "gamma equals Z L minus Z 0 over Z L plus Z 0"

def test_speak_math_swr_from_gamma():
    assert speak_math("$\\mathrm{SWR} = \\frac{1 + |\\Gamma|}{1 - |\\Gamma|}$") == \
        "SWR equals 1 plus magnitude of gamma over 1 minus magnitude of gamma"

def test_speak_math_rho():
    assert speak_math("$\\rho = |\\Gamma|$") == "rho equals magnitude of gamma"

def test_speak_math_angular_frequency():
    assert speak_math("$\\omega = 2\\pi f$") == "omega equals 2 pi f"
    assert speak_math("$X_L = \\omega L$") == "X L equals omega L"

def test_speak_math_varphi_speaks_phi():
    assert speak_math("$\\varphi$") == "phi"

def test_speak_math_q_factor():
    assert speak_math("$Q = \\frac{X_L}{R}$") == "Q equals X L over R"
