...
def test_h4_group_headings_render_anchored_but_not_in_toc():
    ...
    assert '<h4 id="ch02-group-e1a-sample-group-heading-with-topics">' in html
    assert 'Group E1A — Sample group heading; with topics' in html
    assert '<h4 id="ch02-group-e1b-another-group-heading">' in html
    assert '####' not in html
    ...
    assert 'Group E1A' not in toc


def test_txt_strips_h4_group_headings():
    txt = build_txt([pathlib.Path("tests/fixtures/ch_h4_sample.md")])
    assert '####' not in txt
    assert 'Group E1A — Sample group heading; with topics' in txt


def test_series_bar_renders_with_current_book_highlighted():
    html = build_html([pathlib.Path("tests/fixtures/ch_sample.md")], {})
    assert 'class="series-bar"' in html           # the bar renders
    # Extra is this book: highlighted, links to its series mount path
    assert '<a class="current" href="/extra/" aria-current="page">Extra</a>' in html
    # Technician and General are both live: plain links — this book
    # completes the series site, no "coming soon" labels remain
...
def test_title_block_links_audiobook_practice_and_flashcards():
    html = build_html([pathlib.Path("tests/fixtures/ch_sample.md")], {})
    # companion apps linked with RELATIVE hrefs from the title block
    ...
    # the extras line sits in the title block, before the TOC
